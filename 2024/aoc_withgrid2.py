...
from grid2 import (
    ...
    vector_to, adjacents, shift, sub, scale, turn, reverse, taxicab, arrow_to_vector, x, y
)
...
def day16_dijkstra(grid, start, end):
    ...
    while queue:
        ...
        new_cost = {}
        for new_dir, delta in [(dir, 0), (turn(dir, LEFT), 1000), (turn(dir, RIGHT), 1000), (reverse(dir), 2000)]:
...
def day15_part1(lines:list[str]):
    # Parse
    empty = lines.index("")
    room = Day15Grid(lines[:empty])
    moves = list(map(arrow_to_vector, ''.join(lines[empty+1:])))
    ...
    for move in moves:
        ...
        if empties:
            ...
            for distance in range(taxicab(robot, empties[0]), 0, -1):
                room[shift(robot, scale(move, distance))] = room[shift(robot, scale(move, distance-1))]
    ...
    return sum([100*y + x for (x, y) in room.find('O')])

def day15_part2(lines:list[str]):
    # Parse
    empty = lines.index("")
    double = {'O': '[]', '@': '@.', '#': '##', '.': '..'}
    room = Day15Grid([''.join(map(double.get, line)) for line in lines[:empty]])
    moves = list(map(arrow_to_vector, ''.join(lines[empty+1:])))
    ...
    for move_num, move in enumerate(moves):
        ...
        if move in {UP, DOWN}:
            ...
            while not (found_all_empty or found_wall):
                next_y = y + move[1] # TODO: y not in scope
                next_row = {(x, next_y) for x in move_cols}

                # Augment with all of the box-halves that we collect
                unmatched_left = {
                    right for pos in next_row
                    if room[pos] == '[' and (right := shift(pos, RIGHT)) not in next_row
                }
                unmatched_right = {
                    left for pos in next_row
                    if room[pos] == ']' and (left := shift(pos, LEFT)) not in next_row
                }
                ...
                for pos in next_row:
                    ...
                    if x(pos) in move_cols:
                        next_room[pos] = room[sub(pos, move)]
                    else:
                        next_room[pos] = '.'
                ...
                move_cols = {x(pos) for pos in next_row if room[pos] != '.'}
                y = next_y
    ...
    # Compute
    debug(f'After all moves complete:\n{room.pretty()}\n')
    return sum([100*y + x for (x, y) in room.find('[')])
...
def does_it_loop2(grid:Grid, start:Pos) -> bool:
    ...
    while True:
        ...
        if sightline:
            ...
            if shift(sightline[-1][0], vector) not in grid:
                return False # (a.k.a. no loop)
            # Otherwise, move forward
            pos = sightline[-1][0]

        vector = turn(vector, RIGHT)
        # Loop check!
